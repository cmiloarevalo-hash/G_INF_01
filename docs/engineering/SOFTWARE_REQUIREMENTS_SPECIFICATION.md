...
El piloto Gemini procesa PDF cuya firma sea legible, imágenes PNG/JPEG con firma correspondiente y TXT/Markdown UTF-8. Los archivos CSV, XLS/XLSX y otros formatos siguen seleccionables pero no se envían ni se convierten. Los planos admitidos continúan siendo PDF (FR-025). Por decisión humana posterior en Issue #12, se admiten **hasta 20 archivos seleccionados y 50.000.000 bytes originales en total**; exceder cualquiera bloquea toda la solicitud con mensaje específico y cero llamadas LLM. Dentro de esos límites, todos los archivos técnicamente legibles se envían juntos en **máximo una solicitud por acción «Analizar»** para obtener directamente `TITLE_STUDY`; sin legibles se hacen cero llamadas. Los límites del producto no garantizan capacidad del payload, contexto, cuota ni calidad del proveedor ([Gemini File input methods](https://ai.google.dev/gemini-api/docs/file-input-methods), [Rate limits](https://ai.google.dev/gemini-api/docs/rate-limits)). El resultado se valida con Zod; el JSON Schema del proveedor no expresa integridad referencial entre elementos y no reemplaza esa validación. La conversión a schema Gemini elimina la declaración Draft y convierte `const` en `enum` sin modificar el validador ejecutable.

La clave Gemini puede ser aportada por el usuario temporalmente: permanece en el estado de la página durante esta sesión y viaja como cabecera del navegador al servidor y de allí al proveedor. En el piloto #12, el propietario puede elegir por nombre uno de hasta tres Secrets de prueba definidos en AI Studio Settings → Secrets, sin exponer el valor al navegador. Sólo el servidor resuelve los alias explícitos; el listado y la ruta `/api/guest/analyze` exigen un código privado del propietario cuando se usa alias. Si no se configura el modo protegido, los alias no funcionan aunque exista la variable automática `GEMINI_API_KEY`. El visitante sin autorización puede seguir usando su propia clave temporal. No se guardan claves en el repositorio, almacenamiento del navegador, logs o almacenamiento persistente fuera de los Secrets del propietario. Las solicitudes al proveedor tienen timeout de 120 segundos; fallos, cuota, respuestas truncadas o JSON inválido producen error sin resultado ficticio. El usuario recibe aviso de que el contenido legible se envía a Google Gemini para procesamiento temporal. El resultado es preliminar y requiere revisión humana; no es asesoría legal ni un Estudio de Títulos jurídicamente válido.
