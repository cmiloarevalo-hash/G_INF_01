...
- **Suite automatizada local:** `PASS` (28/28 pruebas) con respuestas simuladas; `npm ci`, `npm run build` y `git diff --check` pasan. Las tres pruebas nuevas comprueban el rechazo público 403 del listado y ambas rutas de análisis sin código o con código incorrecto, modo deshabilitado por defecto y sin fallback a `GEMINI_API_KEY`, listado de sólo nombres, resolución del Secret en el servidor y conservación de la clave temporal. Smoke de `npm start` en producción: `/` 200, `/api/health` 200 y listado público de alias 403. Esta evidencia no acredita acceso real a Gemini ni la seguridad del despliegue Starter Tier.
