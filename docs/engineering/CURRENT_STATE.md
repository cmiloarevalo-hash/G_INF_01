...
- **Prueba manual real con Gemini y revisión humana del `TITLE_STUDY`:** pendiente. Las pruebas automatizadas usan respuestas simuladas; no prueban acceso/tier, cuota real, extracción real, diferencias reales ni calidad jurídica. V-005, V-011, V-020, V-023 y V-038 permanecen `NOT RUN` hasta la prueba humana con su propia clave y documentos en Codespaces, vinculada al SHA del PR.
...
- Configuración global/selección de proveedores LLM (Gemini, OpenAI, Anthropic, OpenRouter) permanece `NOT_INTEGRATED`. El flujo piloto #12 integra únicamente Gemini `gemini-3.6-flash` con clave de sesión del usuario; no añade ajustes globales ni cambia silenciosamente proveedor/modelo.
...
Evidencia automatizada del Work Item #12 (sólo proveedor simulado):
- **Transporte/orquestación Gemini:** las pruebas verifican modelo fijo `gemini-3.6-flash`, MIME/contenido PDF e imagen, texto decodificado, archivo individual por solicitud, clave fuera del cuerpo JSON, consolidación de discrepancias, rechazo de XLSX, omisión de fuente, JSON inválido, 429 y timeout controlados. La conversión del schema quita `$schema` y sustituye `const` por `enum`; Zod sigue aplicando validación completa. No se usó una clave real.
- **Suite automatizada local:** `PASS` (25/25 pruebas) con respuestas simuladas; `npm ci`, `npm run build` y `git diff --check` pasan. Smoke `npm start`: `/api/health` 200, `/` 200 y endpoint de extracción sin clave 400. Esta evidencia no acredita acceso real a Gemini.
- **Límites y política:** firma PDF/PNG/JPEG, texto UTF-8, PDF hasta 50 MiB y archivos inline hasta 70 MiB por límite de payload Base64 de Gemini (100 MB). Los ficheros no admitidos se conservan y no se envían.
- **Evidencia humana real:** `NOT RUN`; la prueba manual en Codespaces con clave y archivos propios sigue pendiente. No declarar V-005/V-011/V-020/V-023/V-038 `PASS` desde mocks.
- **CI:** `NOT CONFIGURED` (sin workflow de GitHub Actions). La ejecución local de npm es evidencia automatizada, no aprobación de Supervisor.
